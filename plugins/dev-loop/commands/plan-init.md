...
- `.claude/dev-state.json` … 状態台帳（何が起きたか・設定）。`/step` が読み書きする。**git 管理しない（`.gitignore` に登録・ローカル/開発者ごと。多人数開発のマージコンフリクトを避けるため）**。
...
5. **計画ファイルのパス生成 → ステップ分解**:
   ...
   - **`.gitignore` 登録（state を git 管理外に）**: リポジトリ root（`git rev-parse --show-toplevel`）の `.gitignore` に `.claude/dev-state.json` を登録する。
     - **冪等追記**: `git check-ignore .claude/dev-state.json` が成功（＝既に無視されている。`.claude/` 等の別パターン含む）なら追記しない。無視されていなければ `.claude/dev-state.json` 行を追記する（`.gitignore` が無ければ作成。末尾に改行が無ければ改行してから追記）。
     - **移行**: `git ls-files --error-unmatch .claude/dev-state.json` が成功（＝既に追跡済み）なら `git rm --cached .claude/dev-state.json` で index から外す（作業ファイルは残す。`.gitignore` は追跡済みファイルを無視しないため）。失敗（未追跡・新規プロジェクト）なら何もしない。
...
6. 生成後、ステップ数・ゲート・**レビュー設定（有効/観点数/maxRounds）**・ブランチ・ベースライン結果を要約し、
   ...
   この `plan-init` 自体はコミットしない。生成した計画ファイル（`.claude/dev-plans/<date>-<slug>.md`）と `.gitignore` の変更
   （＋既追跡だった場合の `dev-state.json` の index 削除）は未コミットのまま残るが、最初の `/step` が前提チェックでこれらの
   未コミットを許容し、ステップ完了時の `git add -A` で実装変更とまとめて取り込む（＝計画ファイルは初回 `/step` のコミットに必ず含まれる）。
   `dev-state.json` は `.gitignore` 済みなので `git add -A` でも staged されず、コミットされない（ローカルのまま）。
