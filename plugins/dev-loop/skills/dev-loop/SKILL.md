...
[完了時]
 12. 収束時: status=done(見送りは deferred＋理由), completedAt, verification(gates＋review), learnings を記録 → 1 コミット
     収束失敗時: status=blocked, verification.review を記録, コミットせず作業ツリー dirty のまま停止・報告。out-of-scope must は steps[] に新規ステップ＋globalLearnings に追記
 13. 収束時のみ currentStep を次へ、lastUpdated を更新
```
...
| 最終結果 | 判定 | 次アクション |
|---|---|---|
| `blockOn` 以上の未解決なし（最初から clean / 修正後 clean） | 合格 | `status=done`、`verification.review` 記録、**収束時に 1 回だけ**コミット（メッセージ末尾に `[review: <verdict>, <rounds>r]`）、`currentStep` 前進 |
