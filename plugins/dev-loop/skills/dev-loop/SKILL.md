---
name: dev-loop
description: Reference for the dev-loop harness — versioned plan files (.claude/dev-plans/) + a local git-ignored state ledger (dev-state.json) + /step loop protocol and its state schema. Load when working with dev-loop plans or the /step, /plan-init, /plan-status commands, or when designing a step-by-step, verified, resumable remediation/build plan for any project.
---
...
| コマンド | 役割 |
|---|---|
| `/plan-init [ゴール/仕様/レビュー文書]` | 計画を `.claude/dev-plans/<date>-<slug>.md` に**新規生成（上書きしない）** + `dev-state.json` を生成し `.gitignore` へ登録（＝git 管理外）。ゲート自動検出・ベースライン記録・レビュー設定。 |
| `/step [id]` | 1 ステップ実行（依存確認→実装→ゲート→**階層的レビューゲート**→state 更新(ローカル)→計画+実装を 1 コミット）。空なら次の pending。 |
...
[開始前]
  ...
  3. branch があればそのブランチ上＆作業ツリー clean を確認（違えば停止。計画ファイル(planDoc)や初回の .gitignore が未コミットなのは正常＝clean 判定から除外。dev-state.json は .gitignore 済みで元から git status に出ない。※mid-review 再開時の dirty は「階層的レビューゲート」節を参照）
...
[完了時]
 12. state を最終形まで一括更新する（currentStep/lastUpdated も含めて）。dev-state.json は git 管理外＝コミットされない（ローカルのみ）。
 ...
 13. 収束時のみ 1 ステップ=1 コミット（このステップの最後の操作）: git add -A で planDoc(計画ファイル)＋実装をステージ（dev-state.json は .gitignore 済みで staged されない＝ローカルのまま。git commit -am や部分 add は使わない）→ commit → 直後に git status --porcelain で clean を確認（ignored な dev-state.json は表示されない。計画ファイルの残差分はコミット漏れ＝追従コミットで取り込む）
```
...
| 最終結果 | 判定 | 次アクション |
|---|---|---|
| `blockOn` 以上の未解決なし（最初から clean / 修正後 clean） | 合格 | state を最終形へ（`status=done`、`verification.review` 記録、**`currentStep` 前進・`lastUpdated` 更新まで最終形に**）→ `git add -A` で planDoc(計画ファイル)＋実装をステージ（dev-state.json は git 管理外で staged されない）→ **収束時に 1 回だけ**コミット（メッセージ末尾に `[review: <verdict>, <rounds>r]`）→ `git status --porcelain` で clean 確認（ignored な state は出ない） |
